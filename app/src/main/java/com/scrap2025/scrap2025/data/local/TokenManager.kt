...
constructor(
    ...
    companion object {
        ...
        private val SNS_TYPE_KEY = stringPreferencesKey("sns_type")
    }
    ...
    val snsType: Flow<SnsType?> =
        context.authDataStore.data.map { preferences ->
            preferences[SNS_TYPE_KEY]?.let { value ->
    ...
    suspend fun saveSnsType(snsType: SnsType) {
        context.authDataStore.edit { preferences -> preferences[SNS_TYPE_KEY] = snsType.value }
    }
    ...
    suspend fun clearTokens() {
        context.authDataStore.edit { preferences ->
            ...
            preferences.remove(SNS_TYPE_KEY)
        }
    }
}
